...
exports.getAll = (page) => {
  return pool.query(
    `SELECT course._id, course_name, teacher_id,user_name as teacher_name,subject.subject_name as subject_name, subject.color as color, time_start, time_end, day_study, TO_CHAR(day_start, 'yyyy-MM-DD') as day_start, curr_state, course.status FROM course,"user" ,subject
...
exports.searchCourse = (q, page, stateArr) => {
  return pool.query(
    ...
    [stateArr, q, limit, parseInt(page) * limit]
  );
};
...
exports.startCourse = (courseId, teacerId) => {
  return pool.query(
    `UPDATE course  SET curr_state = 1 WHERE _id=$1 AND teacher_id = $2;`,
    [courseId, teacerId]
  );
};
exports.endCourse = (courseId) => {
  return pool.query(`UPDATE course SET curr_state = 2 WHERE course_id=$1;`, [
    courseId,
  ]);
};

exports.banCourse = (courseId) => {
  return pool.query(`UPDATE course SET status = false WHERE _id=$1;`, [
    courseId,
  ]);
};

exports.unBanCourse = (courseId) => {
  return pool.query(`UPDATE course SET status = true WHERE _id=$1;`, [
    courseId,
  ]);
};

exports.getAllList = () => {
  return pool.query(`SELECT course._id, course_name, teacher_id,user_name as teacher_name,subject.subject_name as subject_name, subject.color as color, time_start, time_end, day_study, TO_CHAR(day_start, 'yyyy-MM-DD') as day_start, curr_state, course.status as status FROM course,"user" ,subject
  WHERE course.teacher_id = "user"._id 
  AND course.subject_id = subject._id;`);
};
